...
class Trade(models.Model):
    ...
    child = models.ForeignKey('Trade', verbose_name='Trade history', on_delete=models.PROTECT, null=True, blank=True)

    def save(self, *args, **kwargs):
        if self.status == "Sold out" and not self.closed:
    ...
    def clean(self):
        ...
        try:
            ...
            validate_id = validate_unique_id_trade(self.uuid, self.id, self.seller,
                                                   self.crypto, self.amount, self.price, self.child)
            if validate_id:
